...
(function() {
  ...
  function setThemeIcon(target) {
    if (target && (target.id === ICON_TOP_ID || target.id === ICON_SIDE_ID)) {
      ...
      const dark = target.id === ICON_TOP_ID ? ICON_DARK_MOON : ICON_LIGHT_MOON;
      const light = target.id === ICON_TOP_ID ? ICON_LIGHT_SUN : ICON_LIGHT_SUN;
      target.classList.add(localStorage.getItem('theme') === 'light' ? dark : light);
    }
  }
